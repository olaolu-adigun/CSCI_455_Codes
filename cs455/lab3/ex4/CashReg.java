...
public class CashReg
{
   ...
   */
   ...
   {
      purchase = purchase + (int)(Math.round(amount * DOLLAR_VALUE));
   }
   ...
   */
   ...
   /**
      ...
      @param change object containing number of dollars, quarters, dimes, nickels, and pennies 
   */
   ...
   */
    ...
    public Change giveChange()
   {
      ...
      sum = sum + (numDollars * DOLLAR_VALUE); 
      int numQuarters = (change - sum) / QUARTER_VALUE;
      sum = sum + (numQuarters * QUARTER_VALUE);
      int numDimes = (change - sum) / DIME_VALUE;
      sum = sum + (numDimes * DIME_VALUE);
      int numNickels = (change - sum) / NICKEL_VALUE;
      sum = sum + (numNickels * NICKEL_VALUE);
      int numPennies = change - sum;
